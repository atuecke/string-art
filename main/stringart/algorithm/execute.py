...
def create_string_art(first_anchor: int, base_img: BaseImage, string_art_img: StringArtImage, line_pixel_dict: dict, line_darkness_dict: dict, iterations: int, cost_method: CostMethod = CostMethod.MEAN, max_darkness: float = None, eval_interval: int = None, importance_map: ImportanceMap = None, gpu_accel = False, use_prev_anchor: bool = True, random_neighbor: bool = False):
    if gpu_accel:
        ""
    else:
